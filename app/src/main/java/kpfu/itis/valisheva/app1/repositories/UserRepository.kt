package kpfu.itis.valisheva.app1.repositories

import kpfu.itis.valisheva.app1.entity.User

object UserRepository {
    val users = arrayListOf(
        User(
            1,
            "Анастасия",
            "https://mykaleidoscope.ru/uploads/posts/2021-03/1617051802_3-p-dekor-torta-3.jpg",
            "богиня сырников",
            "Человеколюбивая Анастасия всегда выбирает «помогающую людям» профессию:" +
                    " врач или педагог, стюардесса, социальный работник, журналист или спасатель." +
                    " Она легко располагает к себе людей, но способна проявить строгость при необходимости." +
                    " В коллективе напоминает прилежную гимназистку и может увлечь своим примером." +
                    " Блестяще справляется с деловыми переговорами, но самостоятельный бизнес – слишком тяжелая ноша для нее."
        ),
        User(
            2,
            "Чембер",
            "data:image/jpeg;base64,",
            "лучший кот",
            "Пес с кличкой Чембер обладает волевым характером." +
                    " Чембер очень энергичени целеустремлен, обладает авторитетом среди остальных псов." +
                    " Умеет властно, самодостаточно руководить и весьма требователен к своим подчиненным." +
                    " Внешня холодность, отчужденность и даже кажущаяся агрессивность скрывает жажду душевного тепла," +
                    " но он не умеет этого показать."
        ),
        User(
            3,
            "Аделя",
            "https://i.natgeofe.com/n/9135ca87-0115-4a22-8caf-d1bdef97a814/75552_square.jpg",
            "Чембер - гад",
            "У скромной Адели не хватает нахрапистости, чтобы построить блестящую карьеру." +
                    " Но она может быть незаменимым подчиненным, на трудах которого будет держаться весь коллектив." +
                    " Для этой девушки не существует отговорок и отказов, когда получает распоряжение начальника." +
                    " Независимо от уровня его сложности, она сделает все быстро и безошибочно."
        ),
        User(
            4,
            "Камиль",
            "https://fatcatart.com/wp-content/uploads/2011/02/Malevich_Red_Square-cat-sm.jpg",
            "в душе кот",
            "Профессию выбирает с расчетом, что она обеспечит ему безбедное существование." +
                    " Благодаря наличию качеств лидера умеет организовать свое дело и грамотно управлять коллективом." +
                    " Врач, юрист, спортсмен, журналист – вот те профессии," +
                    " в которых Камиль может себя реализовать наилучшим образом, стать преуспевающим."
        ),
        User(
            5,
            "Ильнас",
            "https://i.pinimg.com/474x/f4/72/06/f47206e14861019d9d1429f8b5a4f72a.jpg",
            "сижу в тик-токе",
            "Что сулит значение имени Ильнас своему обладателю в деловой сфере?" +
                    " Мужчина очень быстро продвигается по карьерной лестнице благодаря упорству," +
                    " трудолюбию, ответственности. Также у него есть одно важное качество: " +
                    "умение заводить нужные связи. Носитель имени предпочитает общаться с умными, " +
                    "успешными и состоятельными людьми, которые нередко составляют ему протекцию."
        )
    )

    fun getUserById(id : Int) : User?{
        for(user in users){
            if(id == user.id){
                return user
            }
        }
        return null
    }
}


